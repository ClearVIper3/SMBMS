...
$(function(){
	...
	$.ajax({
		type:"GET",//请求类型
		url:currentPath+"/bill/getproviderlist",//请求的url
		data:{},//请求参数
		dataType:"json",//ajax接口（请求url）返回的数据类型
		success:function(data){//data：返回数据（json对象）
			if(data != null && data.length > 0){
				var pid = $("#pid").val();
				providerId.html("");//使用精确的选择器
		...
		error:function(xhr, status, error){//当访问时候，404，500 等非200的错误状态码
			console.error("获取供应商列表失败:", status, error, xhr);
